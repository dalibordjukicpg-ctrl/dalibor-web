...
export function TeamHomeMediaEditor({
  ...
  return (
    <AdminPanel
      title="Upoznajte dizajnere — fotografija"
      description="Portret ili fotografija dizajnera/ke u sekciji „O nama“ na početnoj. Tekstove sekcije uređujete u formi ispod."
    >
      <div className="rounded-xl border border-[var(--site-border)] bg-[#fff9f5]/80 p-5 md:p-6">
        <p className="text-sm leading-relaxed text-[#5c524a]">
          <strong className="text-[#3d342c]">Preporučena rezolucija:</strong>{" "}
          portret oko{" "}
          <strong className="text-[var(--site-brand-hover)]">1400 × 1850 px</strong> ili više (omjer
          blizu <strong className="text-[var(--site-brand-hover)]">3 : 4</strong>), JPG ili WebP visoke
          kvalitete (do 12 MB). Veća širina daje oštrij prikaz na velikim i retina
          ekranima.
        </p>

        <div className="mt-6 flex flex-col gap-6 lg:flex-row lg:items-start">
          <div className="relative mx-auto aspect-[3/4] w-full max-w-[280px] shrink-0 overflow-hidden rounded-xl bg-[#f0e6dc] ring-1 ring-[var(--site-border)] sm:max-w-[320px]">
            {url ? (
              <Image
                src={url}
                alt=""
              ...
              <div className="flex h-full min-h-[280px] items-center justify-center px-4 text-center text-sm text-[#8a7b6e]">
                Nije odabrana fotografija — na sajtu se koristi podrazumijevana
                fotografija dizajnerke.
              </div>
            )}
          </div>
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-3">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-[var(--site-border)] bg-white px-4 py-2.5 text-sm font-medium text-[#3d342c] hover:bg-white"
            >
              <ImageIcon className="h-4 w-4 text-[var(--site-brand)]" />
              Izaberi iz medija
            </button>
            <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-[var(--site-border)] bg-white px-4 py-2.5 text-sm font-medium text-[#3d342c] hover:bg-white">
              <Upload className="h-4 w-4 text-[var(--site-brand)]" />
              <input
                type="file"
            ...
            <button
              type="button"
              onClick={() => setMediaId("")}
              className="text-left text-sm text-[#8a7b6e] underline hover:text-[var(--site-brand-hover)]"
            >
      ...
      <p className="mt-4 text-xs text-[#6b5f54]">
        Sve datoteke su u{" "}
        <Link href={adminPath("media")} className="font-medium text-[var(--site-brand-hover)] underline">
          Medijima
        </Link>
        .
      </p>
      ...
      <button
        ...
        disabled={pending}
        onClick={save}
        className="mt-5 rounded-lg bg-[var(--site-brand)] px-4 py-2 text-sm font-medium text-white hover:bg-[var(--site-brand-hover)] disabled:opacity-60"
      >
      ...
      <AdminMediaPicker
        ...
        imagesOnly
        title="Fotografija za „Upoznajte dizajnere“"
      />
    </AdminPanel>
  );
}
