...
import { getMaintenancePublicStateForRequest } from "@/lib/queries/site-globals";
...
export default async function LocaleLayout({ children, params }: Props) {
  ...
  return (
    <div className="relative z-[1] flex min-h-dvh flex-col bg-transparent text-site-ink">
      <SiteHeader locale={raw} s={s} nav={navResolved} />
      {/* Fiksni header + iOS notch — rezervišemo visinu kao na hero -mt. */}
      <div
        className="h-[calc(4.75rem+env(safe-area-inset-top))] shrink-0 md:h-[calc(5.25rem+env(safe-area-inset-top))]"
        aria-hidden={true}
      />
