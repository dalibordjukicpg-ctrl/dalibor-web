 ...
 * RESEND_API_KEY + RESEND_FROM u okruženju; primalac iz CONTACT_FORM_NOTIFY_EMAIL ili DEFAULT_NOTIFY_INBOX.
 */
...
export async function buildContactEmailSummary(
  ...
  const lines = [
    ...
    `Email: ${p.email}`,
    `Telefon: ${p.phone}`,
    p.inquiryType ? `Vrsta projekta: ${p.inquiryType}` : null,
    "",
    "Poruka:",
...
<body style="font-family:Segoe UI,Arial,sans-serif;color:#1a1208;line-height:1.5;max-width:560px">
  ...
  <table style="border-collapse:collapse;width:100%;font-size:14px">
    ...
    <tr><td style="padding:6px 0;color:#666">Vrsta projekta</td><td style="padding:6px 0">${escapeHtml(p.inquiryType?.trim() || "—")}</td></tr>
  </table>
  <p style="margin:20px 0 8px;font-size:13px;color:#666">Poruka</p>
  <div style="background:#f4f4f5;border:1px solid #e4e4e7;border-radius:4px;padding:12px 14px;font-size:14px;white-space:pre-wrap">${msg}</div>
  <p style="margin:20px 0 0;font-size:12px;color:#888">PDF u prilogu (A4) — svi podaci iz forme.</p>
</body>
</html>`;
}
