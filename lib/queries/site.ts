...
import { getAboutSectionImageUrl, getSiteBranding } from "@/lib/queries/site-globals";
...
export function mergeSiteStrings(
  ...
  const base = {
    ...SITE_STRING_DEFAULTS[defaultLocale],
    ...SITE_STRING_DEFAULTS[locale],
  } as Record<SiteStringKey, string>;
...
export const getSiteLayoutData = cache(async (locale: Locale) => {
  ...
  try {
    aboutImageUrl = await getAboutSectionImageUrl();
  } catch (e) {
    console.error("[getSiteLayoutData about image]", e);
  }
