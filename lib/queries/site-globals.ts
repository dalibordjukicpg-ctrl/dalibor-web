...
import { FALLBACK_ABOUT_PORTRAIT_REL } from "@/lib/clinic-assets";
...
/** Portret za sekciju „Upoznajte dizajnere“ — CMS polje `teamM1MediaId`. */
export const getAboutSectionImageUrl = cache(async (): Promise<string> => {
  try {
    const g = await getSiteGlobalsRow();
    const fromCms = await resolveMediaPublicUrl(g?.teamM1MediaId);
    return fromCms ?? FALLBACK_ABOUT_PORTRAIT_REL;
  } catch (e) {
    console.error("[getAboutSectionImageUrl]", e);
    return FALLBACK_ABOUT_PORTRAIT_REL;
  }
});

/** @deprecated Koristite `getAboutSectionImageUrl`. */
export const getTeamHomePortraitUrls = cache(async (): Promise<string[]> => {
  const url = await getAboutSectionImageUrl();
  return [url];
});
